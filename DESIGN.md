...
- [ ] Etap 10 (PLANOWANE, nierozpoczęte): **tryby wizualizera + adaptacja do gatunku + sync z kolorami okładki** — wizja usera po zobaczeniu projectM na żywo, konkretna lista z rozmowy:
  - Trzy tryby do wyboru: **Ambient** (miękkie światło, rozmyte gradienty, wolne ruchy — proponowany domyślny, razem z okładką i falą reagującą na częstotliwości), **Spectrum** (klasyczne widmo częstotliwości, 32-64 pasma, precyzyjne), **Particle** (tysiące cząsteczek, bas = fale cząsteczek, wysokie = iskry — najbardziej efektowne na tablecie).
  ...
  - Synchronizacja z kolorami okładki albumu (mamy już `AlbumArtColorExtractor`/Palette API z Etapu 1) — dynamiczne tło ambient z dominujących kolorów, płynne przejście palety przy zmianie utworu.
...
- [ ] Etap 12 (PLANOWANE, nierozpoczęte): **wybór chmury, z której się słucha — multi-cloud zamiast tylko Google Drive**. Wizja z rozmowy:
  ...
  - **Prerekwizyt schematu (zgłoszony w rozmowie, potwierdzony jako realny problem)**: `Track.id: Long` (domain) i `TrackEntity`/lokalne id z MediaStore nie skalują się czysto na N źródeł. Już teraz jest tego pierwszy objaw — utwory z Google Drive (Etap 6) dostają Long id przez `hash(fileId) + 10^12`, żeby nie kolidować z lokalnymi `_ID` z MediaStore; to działa dla dwóch źródeł, ale dla 5+ (OneDrive, Dropbox, NAS...) każde z własnym schematem id, ręczne przesunięcia zaczną się gryźć. Przed dodaniem trzeciego źródła w Etapie 12 warto rozważyć złożony klucz/osobną encję źródła (`sourceId` + `remoteId` zamiast jednego globalnego `Long`) zamiast kolejnych hashowych obejść — taniej to zrobić raz, zanim `TrackAffinityEntity`/`TrackCooccurrenceEntity`/historia odtwarzania też zaczną się na tym opierać.
